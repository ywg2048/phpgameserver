<?php
/**
 * TencentCBT4環境AndroidWechat定数（全体）.
 */
class GlobalEnv extends BaseEnv {

  const ENV = 'padctencentcbt4adrwx';
  // CSVのprefix
  const ENV_PREFIX = 'tencentcbt4adrwx';

  // リージョン
  const REGION = 'JP';

  // DB
  const SHARE_DSN			= "mysql:host=cbt4_db; dbname=db_zlmc_share_pr2; port=10000;";
  const SHARE_READ_DSN		= "mysql:host=cbt4_db; dbname=db_zlmc_share_pr2; port=10000;";
  const SERIAL_DSN			= "mysql:host=cbt4_db; dbname=db_zlmc_share_pr2; port=10000;";
  const LOG_DSN				= "mysql:host=cbt4_db; dbname=db_zlmc_log_pr2; port=10002;";
  const LOG_READ_DSN		= "mysql:host=cbt4_db; dbname=db_zlmc_log_pr2; port=10002;";
  const CARD_LOG_DSN		= "mysql:host=cbt4_db; dbname=db_zlmc_log_pr2; port=10002;";
  const DUNGEON_LOG_DSN		= "mysql:host=cbt4_db; dbname=db_zlmc_log_pr2; port=10002;";
  ...
  const DB_HOST				= "cbt4_db";
  const DB_NAME				= "db_zlmc_share_pr2";
  const LOG_DB_NAME			= "zlmc";

  // #PADC# ----------begin----------
  // TlogDB
  const TLOG_DB_DSN			= "mysql:host=cbt4_db; dbname=db_zlmc_online; port=10000;";
  ...
  public static function getReadDbList() {
    return array(
      'mysql:host=cbt4_db; dbname=db_zlmc_user_pr2; port=10001;' => 1,
    );
  }
