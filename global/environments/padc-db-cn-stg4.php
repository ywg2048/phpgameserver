...
class GlobalEnv extends BaseEnv {
  ...
  const ENV = 'padcstg4';
  // CSVのprefix
  const ENV_PREFIX = 'stg4';

  // リージョン
  const REGION = 'JP';

  // DB
  const SHARE_DSN			= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_share";
  const SHARE_READ_DSN		= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_share";
  const SERIAL_DSN			= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_share";
  const LOG_DSN				= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_log";
  const LOG_READ_DSN		= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_log";
  const CARD_LOG_DSN		= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_log";
  const DUNGEON_LOG_DSN		= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_log";
  ...
  const DB_NAME				= "stg4_share";
  const LOG_DB_NAME			= "padcn";

  // #PADC# ----------begin----------
  // TlogDB
  const TLOG_DB_DSN			= "mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_db_zlmc_online";
  ...
  // Memcache キーのprefix
  const MEMCACHE_PREFIX = 'padc_stg4_';
  const MEMCACHE_ADMIN_PREFIX = 'padc_stg4_admin_';
...
//  const REGISTERED_DB_IDS = "1";// PADC版では振り分け方法変更するため未使用
  ...
  public static function getReadDbList() {
    return array(
      'mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_user1' => 1,
      'mysql:host=padc-stg2-rds.cbr34bznu78c.ap-northeast-1.rds.amazonaws.com; dbname=stg4_user2' => 2,
    );
  }
