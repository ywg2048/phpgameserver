 ...
 * 8. ダンジョン潜入
 */
class SneakDungeon extends BaseAction {
	...
	public function action($params){
		...
		if(Env::ENV != 'padclocal') {
			// エンコード文字列を変更された場合のため、デコードパラメータがちゃんと存在してるかチェック
			if (!is_array($this->decode_params)) {
				return json_encode(array('res' => RespCode::FAILED_SNEAK));
			}
		}
		// #PADC# ----------end----------
		...
		$total_power = (isset($params['total_power']) ? $params['total_power'] : 0);
                ...
                if ($token) {
	            $user->reportScore(Tencent_MsdkApi::SCORE_TYPE_TEAM_POWER, $token, $total_power);
                }
		// #PADC# memcache→redis
		...
		if(array_key_exists("dung", $params) && array_key_exists("floor", $params) && array_key_exists("time", $params)) {
			...
			$key = CacheKey::getSneakDungeon($user->id, $params["dung"], $params["floor"], $sneak_time);
			$res = $rRedis->get($key);
			if(!$res){
				// ダンジョン&フロア抽出.
				...
				if($dungeon) {
					...
					list($user, $user_dungeon, $fp, $wave_mons_indexs, $daily_left_times) // #PADC_DY# daily_left_times表示表当日剩余关卡次数
						= UserDungeon::sneak($user, $dungeon, $dungeon_floor, $sneak_time, $helper, $card_id, $card_lv, $skill_lv, $plus, $cm, $curdeck, $rev, false, 0, $total_power, $securitySDK, $player_hp);
				}
				if($user_dungeon) {
					...
					$res = array(
                        ...
                        'daily_left_times' => $daily_left_times // #PADC_DY# daily_left_times表示表当日剩余关卡次数
					);
					...
					//$res['mons'] = $user_dungeon->user_waves;
		...
		if(!$res){
			$res = array(
				...
				'waves' => array(),
				'wave_mons' => array(),
                'daily_left_times' => 0 // #PADC_DY# daily_left_times表示表当日剩余关卡次数
			);
            // レスポンス暗号化 6.5で復活予定.
