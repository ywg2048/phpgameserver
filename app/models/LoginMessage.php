 ...
 * 期限付きログインメッセージ
 */
class LoginMessage extends BaseMasterModel {
  const TABLE_NAME = "login_messages";
  const VER_KEY_GROUP = "li_mes";
    ...
    if($messages){
      foreach($messages as $mes){
        if(static::strToTime($mes->begin_at) <= $time && $time <= static::strToTime($mes->finish_at)){
          ...
          $res['one_mes'] = self::replaceTagMessage($mes->one_message, $area_id);
        }
        if($time < static::strToTime($mes->begin_at) && !isset($res['next_time'])){
          ...
          $res['next_one_mes'] = self::replaceTagMessage($mes->one_message, $area_id);
        }
      }
    }
    return $res;
